...
if st.button('Get data'):
    if stock_name and options:
        try:
            ...
            df_feb26 = pd.read_excel('file6.xlsx')
            #df_mar26 = pd.read_excel('file7.xlsx')
            df_apr26 = pd.read_excel('file8.xlsx')
            df_may26 = pd.read_excel('file9.xlsx')
            ...
            stock_feb26 = find_stock(df_feb26, stock_name.strip())
            #stock_mar26 = find_stock(df_mar26, stock_name.strip())
            stock_apr26 = find_stock(df_apr26, stock_name.strip())
            stock_may26 = find_stock(df_may26, stock_name.strip())
            ...
            for column in options:
                    row = {
                     ...
                     #   'Mar 2026': stock_mar26[column].iloc[0] if len(stock_mar26) > 0 and column in stock_mar26.columns else 'N/A',
                        '10 Apr 2026': stock_apr26[column].iloc[0] if len(stock_apr26) > 0 and column in stock_apr26.columns else 'N/A',
                        '30 May 2026': stock_may26[column].iloc[0] if len(stock_may26) > 0 and column in stock_may26.columns else 'N/A'
                    }
            ...
            names_row = {
                ...
                '10 Apr 2026': stock_apr26['name'].iloc[0] if len(stock_apr26) > 0 and 'name' in stock_apr26.columns else 'N/A',
                '30 May 2026': stock_may26['name'].iloc[0] if len(stock_may26) > 0 and 'name' in stock_may26.columns else 'N/A'
            }
